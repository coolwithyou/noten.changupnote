...
- **현재 실행 중단(2026-08-15 Gate R)**: 사용자 승인으로 `deep-v19` sequence 0~9와 별도 Kordoc sequence 7·3을 실행했고 마지막 runtime은 `paused` generation 115, active lease 0이다. 딥분석 10건은 publishable 9(ready 4·conditional 5), held/deferred 1, repair 4건이며 formal verdict는 `CONTINUE`다. Kordoc sequence 7은 complete·미해결 0, sequence 3은 필드 판정 미해결 0이지만 HWP 쓰기 위치 구조 경고 2건으로 partial 종결했다. 승인 중단 조건에 따라 sequence 6은 착수하지 않았다. `lab:smoke`, `lab:batch` non-dry, `lab:agent --execute`, 자동 대상 선정, 단건/repair, 검수·감사·confirmations의 legacy live 호출은 계속 정적 admission이 차단한다. 다음 모델 실행은 exact 대상과 별도 사용자 승인 뒤에만 진행한다.
