...
- **현재 실행 중단(2026-08-14 Gate R)**: 반복 실패 구조 개선 P0~P3와 exact-next 단건 live Adapter·승인 기반 issuer는 구현·검증됐지만, 현행 Cloud Run worker 증거·사용자 canary 승인·발급된 authority는 아직 없다. `lab:smoke`, `lab:batch` non-dry, `lab:agent --execute`, 자동 대상 선정, 단건/repair, 검수·감사·confirmations의 legacy live 호출은 정적 admission이 차단한다. 일반 허용 범위는 dry-run, plan-only, `createPlan/replay`, proposal-only `lab:experiment:prepare`다. runtime lease나 환경변수로 우회하지 않는다.
...
- 현재 일반 허용 명령은 `lab:batch -- --dry-run`, `lab:agent` plan-only, `lab:experiment:test`, `lab:experiment:prepare -- --series=deep-v18` 같은 모델 무호출·proposal-only 준비 경로뿐이다. `lab:experiment:issue`는 사용자가 exact canary를 승인한 뒤에만 approval SHA를 받아 현행 운영 증거와 paused runtime을 검증하고 단건 authority를 발급한다. 과거 `--max-cost-usd`는 구독에서 1회 경고 후 무시되며 active 실행 정책이나 스냅샷에는 저장하지 않는다. 승인된 live 실행은 legacy batch가 아니라 exact plan/receipt-bound `lab:experiment` Adapter가 한 target씩 소유한다.
