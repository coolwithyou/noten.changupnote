...
## 계획 중심 개발과 검증 선택 (2026-09-23 사용자 결정)

- 개발 순서는 현재 종합 실행계획의 기능 목표와 인수 기준을 따른다. 매 단계 전체 테스트, 집계 gate, 테스트 영수증 작성을 개발 진행 조건으로 요구하지 않는다.
- `test`·`test:product-journey` 일괄 실행 명령과 Matching CI는 제거했다. 같은 역할의 강제 하네스를 새로 만들지 않는다.
- 개별 테스트와 격리 DB·브라우저 도구는 필요할 때 선택해서 사용한다. 변경한 동작을 직접 확인하는 최소 검증을 수행하고 결과와 미검증 사항을 간단히 보고한다.
- 공유 계약·DB 변경도 영향받는 동작과 소비 경로를 검증하되, 고정된 프로젝트 전체 집계 실행을 의무화하지 않는다. 변경 없는 검증을 반복하지 않는다.
- 과거 문서의 필수 집계 gate·모델별 반복 검수·별도 검증 문서 요구보다 이 규칙을 우선한다. 과거 검증 기록은 역사 증거로 보존한다.
- 사용자 답변 보존, 인증·권한, 원문 의미와 출처의 무결성은 서비스 동작 계약이다. 개발 하네스 제거를 이유로 이 로직을 제거하지 않는다. 운영 배포·서비스 쓰기·모델 실행의 기존 승인 범위도 유지한다.
