...
- **현재 실행 중단(2026-08-14 Gate R)**: 반복 실패 구조 개선 P0~P3, exact-next 단건 live Adapter·승인 기반 issuer, 비정상 종료의 상태별 runtime recovery와 현행 Cloud Run worker 안전 상태 확인은 구현·검증됐지만, 사용자 canary 승인·발급된 authority는 아직 없다. `lab:smoke`, `lab:batch` non-dry, `lab:agent --execute`, 자동 대상 선정, 단건/repair, 검수·감사·confirmations의 legacy live 호출은 정적 admission이 차단한다. 일반 허용 범위는 dry-run, plan-only, `createPlan/replay`, proposal-only `lab:experiment:prepare`, recovery inspect다. runtime lease나 환경변수로 우회하지 않는다.
...
- 2026-08-14 actual proposal-only 실행은 exact 30건을 성공적으로 봉인했다(proposal `bdc6147f...`, plan `1a381b0f...`). 비중복 기준은 v17 정식 pilot5+expand10 15건과 committed experiment이며, 임의 과거 run·오래된 작업 cohort 전체를 영구 제외해 희소 층을 고갈시키지 않는다. 이 artifact만으로 live 권한은 생기지 않는다.
