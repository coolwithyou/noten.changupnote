...
assert.equal(DEEP_ANALYSIS_SYSTEM_PROMPT.includes(DEEP_ANALYSIS_APPLICANT_AND_SCALE_SEMANTICS_RULE), true);
assert.match(DEEP_ANALYSIS_APPLICANT_AND_SCALE_SEMANTICS_RULE, /일반 창업자를 재창업자로만 좁히지 말고/);
assert.match(DEEP_ANALYSIS_APPLICANT_AND_SCALE_SEMANTICS_RULE, /신청 주체 표현만으로 업종 자격을 추론하거나.*industry/);
assert.match(DEEP_ANALYSIS_APPLICANT_AND_SCALE_SEMANTICS_RULE, /'영세업체'.*매출액.*covered_dimensions에 revenue를 넣지 마라/);
...
assert.match(
  DEEP_ANALYSIS_STRUCTURED_FILTER_METADATA_RULE,
  /supt_regin.*서비스 권역 또는 포털 분류.*신청기업 소재지 region criterion을 만들지 마라/,
);
assert.equal(
...
assert.match(
  DEEP_ANALYSIS_ALTERNATIVE_PATH_SCOPE_RULE,
  /입주기업, 졸업기업.*예비·초기.*별도 전역 필수조건.*콘텐츠 제작 사업자 또는 제주 거주 개인 창작자/,
);
assert.match(
  DEEP_ANALYSIS_ELIGIBILITY_RANKING_SEPARATION_RULE,
