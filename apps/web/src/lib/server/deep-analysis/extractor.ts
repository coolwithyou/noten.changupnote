...
export const CONFIRMATION_PROMPT_RULES = [
  ...
  "source_span의 기간 시작점·끝점·기준일을 prompt에 무손실 보존한다. 원문이 '2022년 1월 1일부터 공고일까지'이면 '현재까지'나 '오늘까지'로 넓히지 말고 '2022년 1월 1일부터 이 모집공고의 공고일까지'로 묻는다. 입력에서 공고일의 절대 날짜를 확인할 수 없으면 날짜를 추정하지 말고 이 상대 표현을 그대로 사용하며, 기간 끝점을 질문에서 생략하지 마라.",
  ...
  "reusable: 답이 이 공고와 무관하게 성립하는 기업의 사실(특정 항목 수혜 이력, 사업 참여 이력 등)이면 company_fact, 이 공고에서만 유효한 신청 역할 선언(주관기업으로 참여 등)이면 per_notice. 다만 '공고일까지'처럼 공고별 cutoff에 따라 답이 달라지는 질문은 기업 이력이라도 per_notice로 두고 다른 공고에 답을 공용하지 마라.",
...
export const DEEP_ANALYSIS_NON_MATCHING_DECLARATION_RULE =
  "신청서·계획서·제출자료를 허위·거짓·과장 없이 작성한다는 진실성 서약, 표절·도용 금지, 서류 미제출·미비·양식 미준수 같은 접수 절차는 회사가 현재 보유한 자격 사실이 아니다. '중복지원 신청을 하지 않겠다'처럼 미래 행동을 약속하는 확약도 현재 또는 과거 수혜 사실이 아니므로 prior_award criterion이나 confirmation으로 만들지 마라. 이런 문구는 other/text_only exclusion, confirmation, condition_found로 만들지 말고 analysis_markdown 또는 program_intent.caution_notes의 신청 체크사항으로만 보존한다. 참가서약서에서 '당사는 중복 지원을 받은 사실이 없다', '당사는 지식재산권 침해·소송 사실이 없음을 확인한다'처럼 사실 부존재를 1인칭으로 확인하더라도, 같은 연속 인용에 그 사실이 신청 제외·지원 불가·결격을 만든다는 효과가 명시되지 않으면 exclusion이나 confirmation으로 승격하지 마라. 향후 적발 시 환수·제재·선정 취소 가능성만으로 현재 신청 제외 효과를 추정하지 말고 analysis_markdown 또는 program_intent.caution_notes의 신청 체크사항으로 보존한다. 반대로 같은 근거에 신청 제외 효과가 명시된 사실, 과거 허위 제출로 인해 현재 정부사업 참여제한·제재 중이라는 명시적 상태, 현재 동일 과제 중복참여, 당해연도 타 지원 수혜를 사실로 명시한 결격은 해당 canonical criterion으로 추출한다.";
export const DEEP_ANALYSIS_DOCUMENT_ONLY_ELIGIBILITY_RULE =
  "신청서·서식의 빈칸·체크박스·기업정보 기재란과 제출서류 목록은 정보수집·증빙 요구일 뿐이다. 납세증명서·사업자등록증·보험서류 같은 문서의 제출 요구만으로 신청자격·결격·우대·배점 조건이나 ambiguous 후보를 만들지 마라. 주변 문구나 공고 본문에 그 문서가 증명하는 사실의 필수·제외·우대·배점 효과가 명시되지 않았다면 해당 축은 inspected_no_condition이다. 다만 서식이나 서약 안에서도 해당 사실이 신청자격·신청 제외·우대·배점에 미치는 효과가 문장으로 명시되면 그 사실과 효과를 함께 포함한 연속 인용을 근거로 조건을 추출한다.";
...
export const DEEP_ANALYSIS_SOURCE_LIMITATION_RULE =
  "제공된 입력의 범위 때문에 상세 공고·신청자격·제외대상·우대·평가를 완전히 확인할 수 없다고 판단하면 source_limitations에 기록한다. source 자체의 명시적 참조는 explicit_reference, 제공된 chunk가 요약·평가표 같은 제한된 범위라는 판단은 limited_coverage, 분석 중 확인 근거가 부족함을 스스로 밝히는 경우는 model_disclosure다. source_ref.source_id는 판단의 입력 근거가 실제로 들어 있는 DEEP_ANALYSIS_SOURCE wrapper의 id, source_sha256은 같은 wrapper의 sha256, source_span은 그 제공 chunk의 연속된 exact substring이어야 한다. model_disclosure의 explanation은 모델 판단임을 분명히 하되 그 문장을 source_span으로 꾸며내지 마라. 확보하지 못했다고 추정한 파일을 source_ref로 만들지 말고, '붙임2' 표기만으로 붙임1의 존재·역할·내용을 확정하지 마라. 자격·제외·우대 조건이나 평가항목의 방향·명시된 동점 규칙까지 확인할 수 없는 매칭 사실의 미확인은 scope=eligibility_details다. 이 정보들은 제공됐고 숫자 배점·가중치만 공개되지 않아 평가 영향의 크기를 정밀하게 알 수 없는 경우에만 scope=evaluation_precision을 사용하며 자격 누락으로 취급하지 않는다. explanation에서 평가방식·평가방향·평가항목·명시된 동점 규칙 중 어느 판단 근거도 제공되지 않았다고 밝히면서 scope=evaluation_precision을 만들지 마라. 단순한 배점·가중치 미공개나 '평가기준(안)' 표기를 평가방식·방향 부재로 확대하지 말고, 구체적 평가항목이 없더라도 평가방향이나 명시된 동점 규칙이 제공됐다면 그 근거를 보존한다. 배점이 없어 정확한 우선순위를 계산할 수 없다는 설명도 평가항목 또는 방향이 제공됐다면 evaluation_precision으로 보존한다. 접수 방법·제출 서식 같은 절차만의 미확인은 application_procedure, 그 밖의 비매칭 정보만 other로 둔다. 기존 eligibility_details를 수치 배점 미공개라는 이유만으로 evaluation_precision으로 자동 재분류하거나, 반대로 evaluation_precision을 eligibility_details로 바꾸지 마라. 모순되는 limitation을 설명만으로 다른 scope에 자동 재분류하지 말고, 별도 근거가 없으면 그 limitation을 제외한다. affected_dimensions=null은 영향을 특정하지 못했다는 뜻이며 모든 축을 input_missing으로 바꾸라는 뜻이 아니다. 제공된 입력만으로 자격 범위를 충분히 확인했고 별도 제한 판단이 없으면 빈 배열을 반환한다.";
...
export const DEEP_ANALYSIS_APPLICANT_INDUSTRY_SCOPE_RULE =
  "공고가 신청기업의 산업·사업 분야를 자격조건으로 명시하면 KSIC 코드가 없어도 industry/required/in의 value.tags에 그 의미를 보존한다. 'ICT 중소기업'은 industry tags=['ICT']와 size sizes=['중소기업']로 분리한다. ICT를 특정 세부 업종 하나로 좁히거나 존재하지 않는 KSIC 코드를 생성하지 않는다. 공고의 분야와 회사의 세부 업종 사이 포함관계는 매칭 단계가 판정하며, 코드가 없다는 이유만으로 명확한 산업 범위를 text_only로 남기지 않는다. 선택 가능한 산업 나열은 tags의 OR 목록으로, 예시 목록은 list_semantics=open으로 보존한다. 여러 산업 동시 영위, 특정 기술·제품 보유, 등록·인증·실적 전제, 역할·트랙별 조건이 결합되어 tags만으로 의미를 보존할 수 없으면 industry/text_only와 value.note에 전체 조건을 남긴다. 공고명·사업목적에서만 산업을 언급하고 실제 신청기업의 산업 자격인지 확인할 수 없으면 단정하지 않는다. 지원 과제의 주제·도입 기술·육성하려는 생태계만 언급하면 program_intent일 뿐 업종 조건이 아니다.";
