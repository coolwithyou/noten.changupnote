...

{
  ...
  const result = await createDeepRepairProposalPreparer(deps).prepare({ seriesId: "deep-v32" });
  ...
  assert.deepEqual(
    ...
    "첫 15건이 deep-v32 비중복 모집단의 세 필수 층을 모두 포함해야 한다",
  );
  ...
  assert.equal(result.seriesMarkerPath, "spike-out/analysis-lab/experiments/series/deep-v32.json");
  ...
  assert.equal(proposal.policy.seriesId, "deep-v32");
  assert.equal(proposal.policy.seed, 20260909);
...

{
  const first = setup();
  const second = setup();
  const firstResult = await createDeepRepairProposalPreparer(first.deps).prepare({ seriesId: "deep-v32" });
  const secondResult = await createDeepRepairProposalPreparer(second.deps).prepare({ seriesId: "deep-v32" });
...

{
  ...
  await assert.rejects(
    createDeepRepairProposalPreparer(deps).prepare({ seriesId: "deep-v32" }),
...
assert.deepEqual(parseDeepRepairPreparationCliArgs(["--series=deep-v32"]), {
  kind: "prepare",
  seriesId: "deep-v32",
});
assert.deepEqual(parseDeepRepairPreparationCliArgs(["--", "--series=deep-v32"]), {
...
for (const argv of [
  ...
  assert.throws(() => parseDeepRepairPreparationCliArgs(argv), /--series=deep-v32|--help/);
}
...

{
  ...
  try {
    ...
    assert.equal(marker.path, "spike-out/analysis-lab/experiments/series/deep-v32.json");
    await write(marker);
    await write(marker);
    await assert.rejects(
      write({
