...

{
  ...
  const result = await createDeepRepairProposalPreparer(deps).prepare({ seriesId: "deep-v25" });
  ...
  assert.deepEqual(
    ...
    "첫 15건이 deep-v25 비중복 모집단의 네 필수 층을 모두 포함해야 한다",
  );
  ...
  assert.equal(result.seriesMarkerPath, "spike-out/analysis-lab/experiments/series/deep-v25.json");
  ...
  assert.equal(proposal.policy.seriesId, "deep-v25");
  assert.equal(proposal.policy.seed, 20260829);
...

{
  const first = setup();
  const second = setup();
  const firstResult = await createDeepRepairProposalPreparer(first.deps).prepare({ seriesId: "deep-v25" });
  const secondResult = await createDeepRepairProposalPreparer(second.deps).prepare({ seriesId: "deep-v25" });
...

{
  ...
  await assert.rejects(
    createDeepRepairProposalPreparer(deps).prepare({ seriesId: "deep-v25" }),
...
assert.deepEqual(parseDeepRepairPreparationCliArgs(["--series=deep-v25"]), {
  kind: "prepare",
  seriesId: "deep-v25",
});
assert.deepEqual(parseDeepRepairPreparationCliArgs(["--", "--series=deep-v25"]), {
...
for (const argv of [
  ...
  assert.throws(() => parseDeepRepairPreparationCliArgs(argv), /--series=deep-v25|--help/);
}
...

{
  ...
  try {
    ...
    assert.equal(marker.path, "spike-out/analysis-lab/experiments/series/deep-v25.json");
    await write(marker);
    await write(marker);
    await assert.rejects(
      write({
