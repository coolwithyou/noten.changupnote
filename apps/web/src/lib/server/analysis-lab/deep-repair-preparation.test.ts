...

{
  ...
  const result = await createDeepRepairProposalPreparer(deps).prepare({ seriesId: "deep-v18" });
  ...
  assert.equal(result.seriesMarkerPath, "spike-out/analysis-lab/experiments/series/deep-v18.json");
  ...
  assert.equal(proposal.policy.seriesId, "deep-v18");
...

{
  const first = setup();
  const second = setup();
  const firstResult = await createDeepRepairProposalPreparer(first.deps).prepare({ seriesId: "deep-v18" });
  const secondResult = await createDeepRepairProposalPreparer(second.deps).prepare({ seriesId: "deep-v18" });
...

{
  ...
  await assert.rejects(
    createDeepRepairProposalPreparer(deps).prepare({ seriesId: "deep-v18" }),
...
await assert.rejects(
  createDeepRepairProposalPreparer(setup().deps).prepare({ seriesId: "deep-v19" }),
  /deep-v18/,
);
...
assert.deepEqual(parseDeepRepairPreparationCliArgs(["--series=deep-v18"]), {
  kind: "prepare",
  seriesId: "deep-v18",
});
assert.deepEqual(parseDeepRepairPreparationCliArgs(["--", "--series=deep-v18"]), {
...
for (const argv of [
  ...
  assert.throws(() => parseDeepRepairPreparationCliArgs(argv), /--series=deep-v18|--help/);
}
...

{
  ...
  try {
    ...
    assert.equal(marker.path, "spike-out/analysis-lab/experiments/series/deep-v18.json");
    await write(marker);
    await write(marker);
    await assert.rejects(
      write({
