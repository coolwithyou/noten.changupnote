...
export async function readTerminalRepairSource(root: string, sourceManifestSha256: string, sourceGrantSha256: string) {
  ...
  if (manifest.source.kind !== "current_inventory" || manifest.source.completedLaunch || manifest.source.terminalRepair) {
    throw new Error("terminal repair 원본은 일반 current inventory launch여야 합니다.");
  }
  const { readCurrentLaunchInventory, assertCurrentInventoryManifestBinding } = await import("./current-inventory-launch");
  assertCurrentInventoryManifestBinding(manifest, await readCurrentLaunchInventory(root, manifest.source.planArtifactSha256));
  ...
  const binding: AnalysisLaunchTerminalRepairBinding = {
    ...
    receiptSha256s: receipts.map(r => r.sha256), originalSequences: selected.map(t => t.sequence),
  };
