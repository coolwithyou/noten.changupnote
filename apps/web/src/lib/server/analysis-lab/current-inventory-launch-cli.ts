...
export function parseCurrentInventoryLaunchArgs(argv: readonly string[]) {
  ...
  if (args.length < 2 || args.length > 3) {
    throw new Error("--grant-ids=<uuid,...> --concurrency=<1..4> [--analysis-mode=primary_and_application|matching_only]가 필요합니다.");
  }
  const values = new Map<string, string>();
  for (const arg of args) {
    const match = /^--(grant-ids|concurrency|analysis-mode)=(.+)$/u.exec(arg);
  ...
  if (grantIds.length < 1 || grantIds.length > 100 || new Set(grantIds).size !== grantIds.length
    ...
    || !/^[1-4]$/u.test(values.get("concurrency") ?? "")
    || (analysisMode !== "primary_and_application" && analysisMode !== "matching_only")) {
    throw new Error("current inventory exact 인자가 잘못됐습니다.");
  }
  return { grantIds, concurrency, analysisMode };
}
