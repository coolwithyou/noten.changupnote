...
const USAGE = "pnpm lab:roundtrip:canary -- --proposal=<sha256> --sequence=<n> --source=<sha256>";
...
export function parseApplicationRoundtripCanaryArgs(argv: readonly string[]): {
  ...
  const sourceSha256 = values.get("--source") ?? "";
  ...
  return { proposalSha256, sequence, sourceSha256 };
}
