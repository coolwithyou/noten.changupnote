// 딥분석 런 artifact와 실행 코어가 공유하는 서버 도메인 계약.
// 과거 dev UI와 분리해 launch/release 경로가 화면 수명주기에 의존하지 않게 한다.
...
export const ANALYSIS_LAB_PROMPT_VERSION = "lab-deep-v28";
 ...
 * v8은 기존 판정 어휘를 바꾸지 않고 서약 사실·평가 진행 방식의 효과 결속을 강화한다.
...
// 과거 ops summary artifact를 읽는 CLI 호환 계약.
...
// ---- 역사 artifact와 CLI 호환 DTO ----
// dev HTTP 라우트는 제거됐다. 아래 타입은 기존 immutable run·review·audit 파일 및
// 오프라인 CLI 소비자가 읽는 구조를 보존하기 위해 유지한다.
