...
export async function prepareCurrentInventoryLaunch(input: {
  ...
  if (input.grantIds.some(id => historySet.has(id))) throw new Error("current inventory에 과거 이력이 포함됐습니다.");
  const before = await readCurrentEligibility(input.grantIds);
  const prepared = [];
  for (const grantId of input.grantIds) prepared.push(await prepareLabAnalysis(grantId));
  const after = await readCurrentEligibility(input.grantIds);
  ...
  const inventory: CurrentLaunchInventory = {
    ...
    seriesId: `current-${kstDayStartUtc(now).toISOString().slice(0, 10).replaceAll("-", "")}`,
  ...
  const final = await readCurrentEligibility(input.grantIds);
  ...
  return { manifest, manifestSha256: launch.sha256, path: launch.path,
    ...
    historicalExcluded: history.length, modelCalls: 0, serviceWrites: 0,
...
export async function verifyCurrentInventoryLaunchTarget(
  ...
  grantId: string,
  readEligibility: (ids: readonly string[]) => Promise<readonly { grantId: string; sourceRevisionSha256: string }[]> = readCurrentEligibility,
) {
  const target = inventory.targets.find(item => item.grantId === grantId);
  if (!target) throw new Error("current inventory 밖의 target입니다.");
  const rows = await readEligibility([grantId]);
...
async function readCurrentEligibility(grantIds: readonly string[]) {
