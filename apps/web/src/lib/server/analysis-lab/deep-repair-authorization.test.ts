...
function fixture() {
  ...
  const seriesMarkerArtifact = canonicalStored(
    seriesMarker,
    "spike-out/analysis-lab/experiments/series/deep-v25.json",
  );
  const approval = {
  ...
  class MemoryRepository implements DeepRepairAuthorizationRepository {
    ...
    readonly seriesMarkers = new Map([["deep-v25", seriesMarkerArtifact]]);
...
{
  const setup = fixture();
  setup.repository.seriesMarkers.delete("deep-v25");
  await assert.rejects(
    setup.issuer.issueApprovedDeepRepairAuthority({
      approvalId: setup.approvalSha256,
      signal: new AbortController().signal,
...
{
  const setup = fixture();
  setup.repository.seriesMarkers.set("deep-v25", stored(
    setup.seriesMarker,
    "spike-out/analysis-lab/experiments/series/deep-v25.json",
  ));
  await assert.rejects(
...

{
  ...
  setup.repository.seriesMarkers.set("deep-v25", canonicalStored({
    ...setup.seriesMarker,
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof DeepRepairAuthorizationError
      && error.code === "proposal_invalid",
    "deep-v25 final marker는 다른 series proposal을 발급할 수 없어야 한다",
  );
  assert.deepEqual(setup.calls, []);
}
