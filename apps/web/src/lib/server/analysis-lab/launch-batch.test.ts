...
test("launch manifest는 inventory drift를 target telemetry로 보존한다", () => {
  ...
  assert.equal(manifest.execution.applicationFieldAnalysisVersion, "kordoc-application-roundtrip-v18");
...
test("정식 launch publishable은 필드 분석 준비도까지 통과해야 한다", () => {
  ...
  const v12Scan = resolveLabBatchRunScan([scanRecord("v12.json", {
    ...v10FieldReference,
    version: "kordoc-application-roundtrip-v18",
  })]);
...
test("독립 검수 합의 결함 재분석은 exact 원본 대상과 RHWP 필드 분석을 함께 봉인한다", () => {
  ...
  assert.equal(repair.execution.applicationFieldAnalysisVersion, "kordoc-application-roundtrip-v18");
