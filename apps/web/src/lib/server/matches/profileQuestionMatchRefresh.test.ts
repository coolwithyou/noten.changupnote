...
const repositories = {
  matches: {
    async saveMatchState(input: { grantId: string }) {
      savedGrantIds.push(input.grantId);
    },
  },
...
assert.deepEqual(refresh, { scope: "company_dimension", plannedCount: 1, savedCount: 1 });
...
assert.deepEqual(noChangeRefresh, { scope: "company_dimension", plannedCount: 0, savedCount: 0 });
