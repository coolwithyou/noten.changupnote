...
if (write && confirmation !== "ARCHIVE_BIZINFO_ATTACHMENTS") {
  throw new Error("--write requires --confirm=ARCHIVE_BIZINFO_ATTACHMENTS");
}
    ...
    for (const candidate of candidates) {
      try {
        const bundle = await archiveGrantAttachments(candidate.selected, {
          ...
          allowFailures: true,
          storage,
          ...(imageOcrAdapter ? { imageOcr: imageOcrAdapter } : {}),
        });
        candidate.entry.raw.attachments = mergeArchivedKStartupAttachments(
          candidate.entry.raw.attachments,
          bundle.attachments,
        );
        const published = await publishBizInfoGrants(db, [candidate.entry], {
