...
export default async function PricingPage() {
  const [session, user] = await Promise.all([getOptionalWebSession(), getOptionalHeaderUser()]);
  const isLoggedIn = session !== null;

  return (
    <main className="min-h-screen bg-background text-foreground">
      <ServiceHeader user={user} links={appHeaderLinks({ currentHref: "/pricing" })} />
