...
export function ActionQueuePanel({ actions }: { actions: ActionQueueItem[] }) {
  return (
    <section className="dashboard-panel action-panel" aria-labelledby="action-queue-title">
      ...
      <div className="action-list">
        {actions.length > 0 ? actions.map((action) => (
          <article key={action.id} className={`action-item ${action.urgency}`}>
            <div className="action-kind">{actionKindLabel(action.kind)}</div>
            <h3>{action.title}</h3>
            <p>{action.reason}</p>
