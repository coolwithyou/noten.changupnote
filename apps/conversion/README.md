...
## hwp2hwpx 트랙 Phase 1 — .hwp 바이너리 → hwpx artifact

설계: `docs/plans/2026-07-08-hwp2hwpx-track.md` (Phase 0 통과 — 자동 3관문 + 한컴 눈검수 확인).

요청 artifact 에 `"hwpx"` 가 포함되고 **입력이 hwp 바이너리(매직 `D0CF11E0`)** 일 때만
`hwp2hwpx` uber jar 로 `.hwpx` 를 생성해 sibling artifact(kind=`hwpx`)로 업로드한다.

- **변환기**: `neolord0/hwp2hwpx`(Apache-2.0) 커밋 핀 `50ae71b` uber jar. Dockerfile
  **멀티스테이지**(스테이지 1 = maven)에서 clone→CLI 래퍼(`java/Main.java`)+오버레이 pom
  (`java/build-pom.xml`, release=8·shade·mainClass) 적용→빌드→런타임 스테이지에 jar 만 COPY
  (`/opt/hwp2hwpx/hwp2hwpx-cli.jar`, `ENV HWP2HWPX_JAR`). 의존 hwplib 1.1.10·hwpxlib 1.0.9 는
  Maven Central 자동 해결.
- **STORE 재포장 정규화(필수)**: hwp2hwpx 는 `mimetype` 을 DEFLATE 로 저장(OWPML 표준 위반).
  변환 직후 `@cunote/core/documents/hwpx-fill` 의 `readHwpxEntries`→`writeHwpx` 로 재포장해
  `mimetype` STORE 첫 엔트리로 정규화한다.
- **비치명 계약**: 변환 실패는 분류된 `outcome` 으로 보고(warning `hwpx_conversion_<outcome>`)하되
  pdf/page_image/markdown 생성을 막지 않는다. 실패 분류: `converter_unavailable`(jar/java 미탑재)·
  `hwp_v3x`·`encrypted`·`distribution`·`timeout`·`conversion_error`·`repack_failed`. 스킵:
  `skipped_already_hwpx`(입력이 PK)·`skipped_not_hwp_binary`.
- **호환**: 웹앱은 아직 `"hwpx"` 를 요청하지 않으므로(`requestedArtifacts` 미포함) 이 스텝은
  건너뛰고 기존 동작에 영향이 없다.
...
```
src/
  types.ts               공용 타입 (ConvertDocumentInput/Result, Phase2ConversionQuality, HwpxConversion*)
  ...
  convert-document.ts    오케스트레이터 (부분 성공 규칙 + hwpx 변환 스텝 주입)
  hwpx-convert.ts        hwp2hwpx jar 실행 + STORE 재포장 정규화 (hwp2hwpx 트랙 Phase 1)
  storage.ts             T4: R2 클라이언트 + buildStorageKey + uploadArtifacts (계획 7장, hwpx kind 포함)
  ...
  main.ts                T6: 서버 프로세스 엔트리 (Dockerfile CMD → dist/main.js). hwpxConvert 주입
  hwp-markdown-adapter.ts @cunote/core 의 convertHwpBufferToMarkdown 주입 어댑터
  index.ts               배럴 export
java/
  Main.java              hwp2hwpx CLI 래퍼 (스파이크 승격, Dockerfile maven 스테이지가 오버레이)
  build-pom.xml          오버레이 pom (release=8 승격 · maven-shade · mainClass)
scripts/
  convert-lib.mjs        의존성 없는 병렬 구현 (샌드박스 검증용, src/*.ts 미러). T4·T5·hwpx 미러 포함.
  hwpx-convert-lib.mjs   src/hwpx-convert.ts 의 plain-node 미러 (core dist import + java spawn)
  ...
  hwpx-convert-test.mjs  hwp2hwpx 트랙 Phase 1 검증: 변환→PK+mimetype STORE 정규화→업로드 경로
...
# hwp2hwpx 트랙 Phase 1 — 변환+STORE 재포장 정규화+업로드 경로 (java + hwp2hwpx jar 필요)
#   jar 미지정 시 spike-out/hwp2hwpx/hwp2hwpx-cli.jar 사용. core dist 빌드 전제.
#   spike-samples/files/*.hwp 표본 → 산출 hwpx 가 PK + mimetype STORE 첫 엔트리인지 단정.
node apps/conversion/scripts/hwpx-convert-test.mjs [jar경로]
...
| `HWP2HWPX_JAR` | `/opt/hwp2hwpx/hwp2hwpx-cli.jar` | hwp2hwpx uber jar 경로(Dockerfile 스테이지 1 산출). 미탑재면 hwpx 변환이 `converter_unavailable` 로 스킵. |
| `HWP2HWPX_JAVA_BIN` | `$JAVA_HOME/bin/java` → `java` | hwp2hwpx 실행 java 바이너리. |
...
# 4) hwp2hwpx jar 탑재 + 변환 확인 (스테이지 1 산출물)
docker run --rm cunote-conversion ls -la /opt/hwp2hwpx/hwp2hwpx-cli.jar   # ~2.5MB
docker run --rm -v "$PWD/spike-samples/files:/samples:ro" cunote-conversion bash -lc \
  'cd /app/apps/conversion && node --input-type=module -e "
   import { convertHwpToHwpx } from \"./dist/hwpx-convert.js\";
   import { readFileSync, mkdtempSync } from \"node:fs\"; import { tmpdir } from \"node:os\"; import { join } from \"node:path\";
   const f=process.env.F; const r=convertHwpToHwpx({ body: readFileSync(f), workDir: mkdtempSync(join(tmpdir(),\"c.\")) });
   const b=readFileSync(r.artifact.path); const nl=b.readUInt16LE(26);
   console.log(r.outcome, b.subarray(0,2).toString(), b.subarray(30,30+nl).toString(), b.readUInt16LE(8));"' \
  -e F=/samples/26_11d604b4b6353493-참여_신청서._개인정보_수집_및_이용_동의서_양식.hwp
#   → "converted PK mimetype 0" (PK 시그니처 + mimetype STORE 첫 엔트리)
```

> **hwp2hwpx 빌드 소요·네트워크 의존**: 스테이지 1(maven)은 `neolord0/hwp2hwpx` clone +
> hwplib/hwpxlib(Maven Central) 다운로드가 필요하다. 로컬 실측(2026-07-08, darwin/amd64):
> clone ~4s + `mvn package` ~9s(캐시 없이) = jar 2548531B. 전체 이미지 빌드는 apt(LibreOffice/폰트)
> 다운로드가 대부분을 차지한다.
