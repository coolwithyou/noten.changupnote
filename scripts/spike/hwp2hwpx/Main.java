...
/**
 * hwp2hwpx 스파이크용 얇은 CLI 래퍼 (Phase 0).
 *
 * 절차: HWPReader.fromFile(in) -> Hwp2Hwpx.toHWPX(hwp) -> HWPXWriter.toFilepath(hwpx, out).
 * 성공/실패를 한 줄 태그로 stdout/stderr 에 내보내 드라이버(mjs)가 파싱한다.
 *   성공: "OK version=<v> compressed=<b> distribution=<b> hasPassword=<b>"
 *   실패: stderr 로 "ERR <예외클래스>: <메시지>" + exit 1
 * 실패 사유 분류(HWP 3.x/암호화/배포용/기타)는 드라이버가 이 메시지 + 매직바이트로 수행한다.
 */
public class Main {
