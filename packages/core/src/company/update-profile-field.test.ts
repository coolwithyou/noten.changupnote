...
check("fuzzy/partial 공개명단 값은 사용자 exact 확인으로 교체 가능", () => {
  const partial: CompanyProfile = {
    region: { code: "11", label: "서울 후보" },
    profile_evidence: {
      region: {
        sourceKind: "public_registry",
        provider: "registry_fuzzy",
        asOf: "2026-07-11T00:00:00.000Z",
        axisCompleteness: "partial",
        confidence: 0.7,
      },
  ...
  const updated = updateCompanyProfileField(partial, {
    field: "region",
    value: { code: "26", label: "부산" },
    confidence: 0.6,
    sourceKind: "self_declared",
  ...
  assert.equal(updated.region?.code, "26");
  assert.equal(updated.profile_evidence?.region?.sourceKind, "self_declared");
});
