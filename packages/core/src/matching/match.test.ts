...
check("검수 전 핵심 조건(needs_review)은 통과해도 확인 필요 게이트로 내린다", () => {
  ...
  assert.equal(result.review_gate?.tier, "needs_core_review");
  assert.equal(result.review_gate?.scoreDisplay, "hidden");
  assert.equal(result.review_gate?.reasons[0]?.code, "unreviewed_criteria");
});
...
check("검수 전 required/exclusion은 비핵심 축의 pass까지 항상 unreviewed_criteria 게이트로 보낸다", () => {
  const matrix: Array<{ label: string; criterion: GrantCriterion; profile: CompanyProfile }> = [
    {
      label: "region/pass",
      criterion: {
        dimension: "region",
    ...
    },
    {
      ...
      criterion: {
        ...
        kind: "required",
        confidence: 0.5,
        source_span: "사업장 소재지 별도 확인",
        value: { note: "소재지 확인 필요" },
        needs_review: true,
      },
      profile: company,
    },
    ...
    {
      label: "biz_age/unknown",
      criterion: {
        dimension: "biz_age",
        operator: "text_only",
        kind: "required",
        confidence: 0.5,
        source_span: "창업기업 업력 별도 확인",
        value: { note: "업력 확인 필요" },
        needs_review: true,
      },
      profile: company,
    ...
    {
      label: "prior_award/pass",
      criterion: {
        dimension: "prior_award",
        operator: "not_in",
        kind: "exclusion",
        confidence: 0.9,
        source_span: "동일 사업 기수혜 기업 제외",
        value: { programs: ["없는 사업"] },
        needs_review: true,
      },
      profile: { ...company, prior_awards: [], list_completeness: { prior_award: "complete" } },
    },
    {
      label: "prior_award/fail",
      criterion: {
        dimension: "prior_award",
        operator: "not_in",
        kind: "exclusion",
        confidence: 0.9,
        source_span: "동일 사업 기수혜 기업 제외",
        value: { programs: ["창업도약패키지"] },
        needs_review: true,
      },
      profile: {
        ...company,
        prior_awards: ["창업도약패키지"],
        list_completeness: { prior_award: "complete" },
      },
    },
    {
      label: "prior_award/unknown",
      criterion: {
        dimension: "prior_award",
        operator: "text_only",
        kind: "exclusion",
        confidence: 0.5,
        source_span: "중복 수혜 여부 별도 확인",
        value: { note: "수혜 이력 확인 필요" },
        needs_review: true,
      },
      profile: company,
    },
    {
      label: "tax_compliance/pass",
      criterion: {
        dimension: "tax_compliance",
        operator: "not_in",
        kind: "exclusion",
        confidence: 0.9,
        source_span: "국세 체납 기업 제외",
        value: { flags: ["national_tax_delinquency"] },
        needs_review: true,
      },
      profile: {
        ...company,
        tax_compliance: {
          flags: [],
          known_flags: ["national_tax_delinquency"],
          exceptions: [],
        },
        confidence: { ...company.confidence, tax_compliance: 0.9 },
      },
    },
    {
      label: "tax_compliance/fail",
      criterion: {
        dimension: "tax_compliance",
        operator: "not_in",
        kind: "exclusion",
      ...
      profile: {
        ...company,
        tax_compliance: {
          flags: ["national_tax_delinquency"],
          known_flags: ["national_tax_delinquency"],
          exceptions: [],
        },
        confidence: { ...company.confidence, tax_compliance: 0.9 },
    ...
    {
      label: "tax_compliance/unknown",
      criterion: {
        dimension: "tax_compliance",
        operator: "text_only",
        kind: "exclusion",
        confidence: 0.5,
        source_span: "세금 체납 여부 별도 확인",
        value: { note: "체납 확인 필요" },
        needs_review: true,
      },
      profile: company,
  ...
  for (const item of matrix) {
    const result = matchGrantCriteria([item.criterion], item.profile);
    assert.equal(result.review_gate?.tier, "needs_core_review", item.label);
    assert.equal(result.review_gate?.scoreDisplay, "hidden", item.label);
    assert.equal(result.review_gate?.reasons[0]?.code, "unreviewed_criteria", item.label);
  }
});
...
/** pass하는 required 1건 + unknown으로 남는 text_only exclusion 1건(id 보유). */
function confirmationFixtureCriteria(): GrantCriterion[] {
  return [
    {
      id: "criterion-region-required",
      dimension: "region",
      operator: "in",
      kind: "required",
      confidence: 0.9,
    ...
    {
      id: "criterion-exclusion-1",
      dimension: "other",
      operator: "text_only",
      kind: "exclusion",
      confidence: 0.8,
      source_span: "국세 체납 등 결격 사유가 있는 기업은 지원 제외",
      value: { note: "결격 사유 제외" },
    },
  ];
}

check("비결격 확인 답변은 exclusion unknown을 pass로 소거하고 eligible로 승격한다", () => {
  ...
  assert.equal(result.eligibility, "eligible");
  const entry = result.rule_trace[1];
  assert.equal(entry?.result, "pass");
  assert.equal(entry?.resolution, "confirmed_by_user");
  assert.match(entry?.message ?? "", /본인 확인/);
...
check("미답변 exclusion criterion은 현행대로 unknown → conditional을 유지한다", () => {
  const result = matchGrantCriteria(confirmationFixtureCriteria(), company);
  assert.equal(result.eligibility, "conditional");
  assert.equal(result.rule_trace[1]?.result, "unknown");
  assert.equal(result.rule_trace[1]?.resolution, undefined);
});

check("빈 confirmations 입력은 미제공과 완전히 동일한 결과를 낸다(회귀 불변식)", () => {
  const withEmpty = matchGrantCriteria(confirmationFixtureCriteria(), company, { confirmations: [] });
  const withoutOption = matchGrantCriteria(confirmationFixtureCriteria(), company);
  assert.deepEqual(withEmpty, withoutOption);
});

check("비-exclusion criterion에 대한 확인 답변은 방어적으로 무시한다", () => {
  const result = matchGrantCriteria(confirmationFixtureCriteria(), company, {
    confirmations: [{ criterion_id: "criterion-region-required", disqualified: true }],
  });
  // required region은 회사 값으로 pass — 자가신고 입력이 이를 fail로 바꾸지 못한다.
  assert.equal(result.rule_trace[0]?.result, "pass");
  assert.equal(result.rule_trace[0]?.resolution, undefined);
  assert.equal(result.eligibility, "conditional"); // exclusion은 미답변 그대로 unknown
});

check("비결격 확인 답변은 실데이터 기반 exclusion fail을 뒤집지 못한다", () => {
  const criteria: GrantCriterion[] = [
    {
      id: "criterion-exclusion-region",
      dimension: "region",
      operator: "in",
      kind: "exclusion",
      confidence: 0.9,
  ...
  const result = matchGrantCriteria(criteria, company, {
    confirmations: [{ criterion_id: "criterion-exclusion-region", disqualified: false }],
  });
  assert.equal(result.eligibility, "ineligible");
  assert.equal(result.rule_trace[0]?.result, "fail");
  assert.equal(result.rule_trace[0]?.resolution, undefined);
