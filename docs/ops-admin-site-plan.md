...
# GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET은 changupnote.com 사용자 프론트와 공유(재사용)한다 — admin 전용 키 없음
...
Google provider는 `changupnote.com` 사용자 프론트와 동일한 `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`를 재사용한다(2026-07-09 변경 — 애초에 문서상 "admin 전용"으로 기록했던 client id가 실제로는 웹 client id와 동일했다). 로그인 허용 범위는 client 분리가 아니라 아래 `signIn` callback의 도메인 검사 + `admin_users` 등록 여부로 제한한다.

Google OAuth 설정(같은 client에 오리진/redirect URI를 추가 등록):

- Authorized JavaScript origins: `https://changupnote.com`, `https://ops.changupnote.com`
- Authorized redirect URI: `https://changupnote.com/api/auth/callback/google`, `https://ops.changupnote.com/api/auth/callback/google`
...
- `NEXTAUTH_SECRET`
- `ADMIN_ALLOWED_GOOGLE_DOMAIN=noten.im`
- `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` (changupnote.com 프론트와 공유)
- `DATABASE_URL`
- `CUNOTE_WEB_DATA_SOURCE=drizzle`
...
- Google provider는 `ADMIN_GOOGLE_CLIENT_ID`, `ADMIN_GOOGLE_CLIENT_SECRET`만 사용한다. (2026-07-09: 실제로는 web과 같은 client였음이 확인되어 `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` 공유로 정리 — 위 "Google 로그인 제한" 절 참고)
