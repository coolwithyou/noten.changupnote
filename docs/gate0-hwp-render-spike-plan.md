...
>
> **🟢 채점 + 2차 확장 검증 (2026-07-02)**
>
> - 1차 채점 (대표 표본 10건, AI 1차 — 2인 교차는 사람 검수로 보완 예정): table 평균 **2.0/2**, layout 평균 **1.6/2**, 빈칸/체크박스/서명란 시각 보존 **100%** → 통과 기준(1.5) 충족
> - 발견된 약점 패턴: 표 밖 제목 텍스트박스/도형에서 간헐적 겹침·잘림 (30건 중 4건, 표 내부 필드는 무영향). overlay 가이드의 핵심인 표 내부는 전 샘플 온전
> - 2차 확장 배치 30건: **pyhwp 변환 실패 10건 전량 + 모집단 무작위 20건** — 렌더링 **30/30 (100%)**. pyhwp가 파싱조차 못 한 파일들도 H2Orestart는 표 구조까지 온전히 렌더링함을 육안 확인
> - **누적 60/60 (100%)** → Gate 0 통과 판정. 한컴 SDK 견적 문의는 보류 (품질 기준 미달 시에만 재개)
> - 확정: 마스터 설계 8.3 렌더러 체인 1차 엔진 = LibreOffice headless + H2Orestart. pyhwp는 텍스트 추출 보조로 강등. 변환 서버(Phase 2)는 이 조합을 컨테이너화
> - 산출물: `spike-out2/report.html`, `spike-samples2/manifest.csv`
> - 다음: Gate 1 라벨링 착수 (`docs/gate1-field-map-labeling-guide.md`), Phase 2 변환 서버 구현 계획 수립 가능
