...
>
> - ✅ ~~[차기 세션 최우선·사용자 지시] 리뷰어 워크스페이스 문서 목록에 공고명 표시~~ (2026-07-08 완료, `9745d23`): `listReviewDocs`에 docRef 배치 해석(`resolveGrantTitles` — surface는 surfaces.grantId, spike는 (source,sourceId))으로 `grantTitle` 부여, 목록 문서 컬럼을 공고명 主·docId 부기로 전환(미해석은 docId 폴백). 상세(`ReviewDocDetail`)는 evidence.grant.title 재사용. 실DB 49건(spike 45·surface 4) 전건 해석 확인, typecheck·build 통과
