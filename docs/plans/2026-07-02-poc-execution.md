...
> - 🔶 **개발 트랙 [C] Gate 2 layout 어댑터** (2026-07-04 착수): ✅ 외부 대조 델타 완료 — 전제 4건 유지, **kordoc은 bbox 미반환으로 text parser 계층 재분류**, 후보 통합 사실표 확정 (`docs/research/2026-07-04-gate2-layout-adapters-calibration.md`). 구현 플랜·위임 스펙: `docs/plans/2026-07-04-gate2-layout-adapters.md`. **통과 판정·캘리브레이션은 golden 쌓인 후** — 검수 15~20건 시점 부분 golden 조기 측정 (2026-07-03 사용자 합의: 병렬). 사용자 준비물: `UPSTAGE_API_KEY`·Azure DI 리소스 키(없는 엔진은 러너가 스킵)
