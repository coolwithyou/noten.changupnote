...
> - ✅ **개발 트랙 [C] Gate 2 layout 어댑터 실행 인프라 완료** (2026-07-04): 외부 대조 델타(전제 4건 유지, kordoc→text parser 재분류) → 어댑터 5종+정규화+러너/캐시+메트릭 구현·검증. `pnpm eval:layout -- --engine <name|all> --docs <ids|all> [--allow-paid] [--write]`. 0030(eval_target에 field_map) 적용됨. kordoc 31/45 실측(후보 1,944건), Google Form Parser 실호출 검증(us 리전 프로세서 프로비저닝됨, `.env.local`의 `GOOGLE_DOCAI_PROCESSOR`). 상세·한계: `docs/plans/2026-07-04-gate2-layout-adapters.md`. **다음**: ① 사용자 키 등록(`UPSTAGE_API_KEY`·Azure — 없으면 스킵) ② golden 15~20건 시점 `--write` 조기 측정 [D] ③ PaddleOCR 로컬 docker 기동(선택)
