...
>
> - ✅ **세션 11 (2026-07-08)** — **이상 플로우 세로 관통 슬라이스 A·B·C** (`docs/plans/2026-07-08-ideal-flow-vertical-slice.md` — 갭 분석 실측: surface 191 전부 pending·artifact 0·grant_document_fields 0·golden 0). ① `ad3d8a8` 슬라이스 A: 변환 폴링 배선 — 공용 스윕 헬퍼(pollSweep) + `/api/cron/poll-conversions`(수동·향후 스케줄) + ingest-kstartup cron 말미 잔여 예산 스윕(Hobby cron 2개 제한 우회) + **on-demand** `POST /api/web/grants/[grantId]/conversions/poll`(공고 상세 진입 시 ConversionPollTrigger가 백그라운드 호출) + 공고 상세 '문서 미리보기' 진입 링크/'변환 중' 배지(뷰어 고아 페이지 해소) ② `be6bcbf` 슬라이스 B: **전문가 보정 브리지** — B1 검수 큐 유입기(`pnpm import:review-docs:from-surfaces`, docRef `surface:<id>`·R2 재업로드 없음·멱등) · B2 LLM 사전라벨(`pnpm prelabel:review-docs`, 순환성 가드: pending 고정·labeledBy=`ai:<model>`) · B3 승인→`applyReconciledFields`(같은 트랜잭션, fields_ready 전이)+취소 롤백. 검수 GUI 무깨짐(surface 근거 로더·이미지 프록시 `grant-convert/` 허용). 실DB 왕복 `pnpm verify:review-surface-bridge` 11체크·잔재 0 ③ 슬라이스 C: support `coaching` 카테고리 + 공고 상세 '도움받기' CTA + `/support` prefill(`?category=&topic=`). **남은 유일 블로커는 A7(env)로 수렴** — 아래 항목 갱신됨
>
> 남음 (우선순위순):
>
...
> - 🔄 **[HWPX 후속] hwp2hwpx 변환 트랙** (2026-07-08 착수) — 외부 대조(`docs/research/2026-07-08-hwp2hwpx-calibration.md`, 유지 3·보강 3) → 설계 확정(**정본: `docs/plans/2026-07-08-hwp2hwpx-track.md`**) → **Phase 0 통과**(`724aab6` + 한컴 눈검수 — 변환 22/22·구조·채움·셀 안착. H2O 렌더 오라클 부적합 실증, kordoc 대체 후보 제외) → **Phase 1 완료**(`4c6f44e` 변환 서버 hwpx sibling artifact, docker 실측) → **Phase 2 완료**(`8d9c61f` web 배선: 요청·플래그·다운로드 합류). **Phase 3 진행**(`097892c` — Cloud Run 재배포 리비전 00005-swk + hwpx sibling 원격 생성 실증 PASS). ✅ answers 동봉 경로 실측 완료(2026-07-08, 바이오스타 2.0 사업계획서 — UI POST 동봉·서버 병합 A/B·정직 보고 3중 실증, HWPX 트랙 소잔여 0). 잔여: 실공고 hwp E2E(신규 job부터 sibling 자연 축적)·커버리지 재측정
>
...
> - 🔄 **[신규 트랙] Apply Experience v2 — 이상적 지원서 작성 경험** (2026-07-09 계획 수립, 레드팀 v2 반영 · **2026-07-10 진행: P0~P2c 완료(커밋 a92c3d2·5b0afcc·bb4c2d9·aa964fa·4767389) — 미니멀 상세 + workspace(필드 컨펌·사다리·HWPX 합류) + 구 기능 이식·/preview 리다이렉트까지 Phase 2 번들 완성. 검수에서 결함 2건 발견·수정(suggested Unfilled 미보고, 파일명↔스토리지키 매칭). **P0~P4 전 Phase 완료**(~42fdda2): 미니멀 상세·workspace(필드 컨펌·사다리·HWPX)·구 기능 이식·그라운딩 채팅(AI SDK v7+Citations)·생성형 필드 제안(Gate3 재대조 반영 — manual 제외·basis 실재 검증). 잔여: 사용자 브라우저 QA·배포 합의. P5·P6 게이트 유지**): 미니멀 공고 상세 + `/grants/[id]/workspace`(HWP 프리뷰·필드 컨펌 채움·그라운딩 채팅·필드 AI 설명) + 매칭 보강 채팅(게이트). 정본: `docs/plans/2026-07-09-apply-experience-v2.md`. **착수 문서: `docs/plans/2026-07-10-apply-experience-v2-handover.md` (트리거 문장 포함, Fable 오케스트레이션 + opus/sonnet 위임)**. ideal-flow 슬라이스 D·E·G 승계
> - ⬜ **[이슈·Apply v2 P0-3 발견] 변환 서버 렌더 이미지에 함초롬 폰트 미탑재** — `apps/conversion/Dockerfile:44-48`은 nanum·noto-cjk만 설치, 함초롬바탕/함초롬돋움(HCR) 부재. HWP가 함초롬 지정 시 폰트 대체로 조용한 레이아웃 밀림 가능. HCR 폰트 라이선스(한컴 배포 조건) 확인 후 Dockerfile 반영 검토 (2026-07-10 실측)
...
> - ✅ ~~[사용자→세션] A7 + 세로 관통 E2E~~ (세션 11 후반 완료, `c7ec6e6`): env 등록(.env.local + Vercel 3환경) → **E2E 결함 2건 발견·수정**(① archive_url=S3 엔드포인트 무인증 400 → presigned GET 공급 ② bizinfo 아카이브 2,234건 surface 0건 → `pnpm backfill:attachment-surfaces` 신설) → open 공고 4건 변환 4/4 succeeded(artifact 72건) → 검수 큐 등재 4건 + 사전라벨 실 LLM 스모크 14필드 → 프로덕션 배포 Ready(cron 라우트 401 가드 확인). **후속**: ⓐ 리뷰팀이 /internal/review에서 실공고 4건(s-*) 검수·승인 → 뷰어 필드 노출 확인 ⓑ backfill 확대 실행(open 전체, `--limit` 상향) ⓒ kstartup 첨부는 robots `/afile*/` Disallow로 크롤 다운로드 불가 — 공공데이터 API 파일 채널 검토(신규 과제) ⓓ `/grants/[id]/preview` 브라우저 왕복(사용자 dev 서버)
...
> - ✅ ~~[E] Phase 3 Viewer v1~~ (세션 6 완료 — 위 완료 목록). 후속 소과제: ✅ ~~진입 링크(ApplySheetView)~~ (세션 11 `ad3d8a8`) ⬜ A7 완료 시 실 conversion artifact로 재검증
