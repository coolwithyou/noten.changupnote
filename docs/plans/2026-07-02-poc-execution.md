...
> - 🔶 **[사용자→세션] A7 + 세로 관통 E2E** (세션 11에서 코드 배선 완료, env만 남음): `CONVERSION_SHARED_SECRET`이 Secret Manager에만 있고 `sw@ba-ton.kr` 권한 없음·`sw@noten.im` 토큰 만료. ① 사용자: `gcloud auth login sw@noten.im` ② 세션: 시크릿 취득(비노출)→`.env.local`+Vercel env(changupnote, prod·preview·dev) 등록→`pnpm conversion:poll -- --write --limit=3` E2E(실데이터 반영 목적이라 검증 행 삭제 불필요)→클린 worktree 배포→`import:review-docs:from-surfaces`→`prelabel:review-docs` 실 LLM 스모크 1문서→/internal/review 승인→`/grants/[id]/preview` 필드 노출 왕복
