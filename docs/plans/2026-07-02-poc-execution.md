...
>
> - ⬜ **[차기 세션 최우선·사용자 지시] 리뷰어 워크스페이스 문서 목록에 공고명 표시** (2026-07-08 사용자 피드백): `/internal/review` 목록의 문서 컬럼이 `doc01`·`s-c9cffb83` 같은 내부 id라 비직관적 — **실제 공고명(grants.title)을 1차 표기**로 노출한다. 구현 힌트: `listReviewDocs`(apps/web/src/lib/server/review/reviewDocsRepo.ts)는 목록용이라 공고 조인이 없음 — spike 문서는 `parseReviewDocRef`→(source, sourceId)→grants, surface 문서는 `surface:<id>`→surfaces.grantId→grants 로 배치 조회해 title을 붙이고, 미해석 문서는 기존 docId 폴백. UI는 features/review 목록 테이블(문서 컬럼 = 공고명 主·docId 부기). 상세 화면 근거 패널(loadReviewDocEvidence)은 이미 공고명을 갖고 있으니 참고
