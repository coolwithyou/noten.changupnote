...
> 🟢 진행 상황 (2026-08-03): v1 구현·검증·커밋 완료(7bd12a2). 4트랙 병렬(러너 추출 A / 집계·계약 B / UI C / 잡·라우트 D) — 신규 테스트 3종+기존 스위트 통과, tsc 0, dry-run 바이트 동일, dev 서버 실측(깔때기 실데이터·배치 라우트 400/409). 잔여: 실 배치 웹 E2E 1회(CLI 배치 완주 후 — 이중 분석 방지로 보류), v2 후보(검수·감사·집계 실행 트리거, SVG 유량 캔버스).
