...
### 7.5 단일 셀 확약서 분해 (2026-08-09)

- G-Bio 지원 양식에서 주소지 이전 확약서 한 장이 하나의 표 셀로 접혀 10개 실제 입력 위치가 거대한 체크박스 후보 하나로 합쳐지는 구조 공백을 확인했다.
- `주소지 이전 확약서`, 기업정보·이전/신규등록 두 섹션, 사업자등록번호·이전예정지역 라벨이 함께 있는 경우에만 회사명·사업자번호·현주소·대표자·사업장 종류·등록 형태·이전 예정 지역·확약일·서명란 2개로 분해한다.
- 라벨 뒤 입력을 위한 `insert_after_label` 쓰기 연산을 추가하고, 동일 셀의 여러 편집은 뒤쪽 위치부터 적용해 원문 좌표를 보존한다. 장문 구조 경고를 숨기거나 완료 기준을 낮추지 않는다.
- 이 필드·쓰기 계약 변경부터 현행 계약은 `kordoc-application-roundtrip-v7`이다. v6 이하 산출물은 현행 선분석으로 재사용하지 않는다.

검증 기준은 접힌 확약서 10개 필드 탐지·10개 동시 쓰기 성공, Opus 5 사용자 입력 재판정, 지원 양식 구조 경고 0, 품질 그래프의 application lane `passed`다.

#### 품질 그래프 피드백 — 신청기업 산업 범위 (2026-08-09)

- Kordoc v7 카나리 재실행에서 application lane은 통과했지만, 같은 원문의 새 딥분석 런이 반복된 `바이오 스타트업` 신청자 범위를 program intent로만 남겨 industry 축을 비웠고 독립 Fable 검수와 Sonnet 감사가 누락으로 차단했다.
- 모집직무·지원주제를 업종으로 오인하지 않는 기존 경계는 유지한다. 다만 공고명·목적·모집안내가 신청자를 동일 산업 기업으로 반복 지칭하고 지원내용·신청 분류도 그 산업에 한정되면 industry/required/text_only로 보존한다.
- KSIC·업태가 없는 문장을 임의 태그로 구조화하지 않아 자동 탈락은 막고, 비대상 산업 기업의 자동 통과도 막는다. 이 경계부터 로컬 prompt는 `lab-deep-v11`, 운영 prompt는 `deep-analysis-v18`, blind audit은 `deep-analysis-blind-audit-v23`이다.
