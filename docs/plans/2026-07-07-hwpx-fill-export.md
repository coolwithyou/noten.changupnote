...
> - ✅ 소잔여 해소(2026-07-08): answers 동봉 경로 실측 — 바이오스타 2.0 공고 사업계획서
>   (business_plan·.hwpx, 추가 입력 2문항)에서 초안 생성 → **재생성 없이** 문항 입력 → HWPX
>   다운로드. 실증 3중: ① UI POST body에 answers 원문 동봉(fetch 후킹으로 확보) ② 서버 병합
>   A/B — answers 전송 시에만 미채움 8→10건으로 두 라벨이 채움 시도 집합에 등장, no_label_cell
>   정직 보고 ③ HTTP 200·파일 스트림 정상. 이 템플릿엔 두 라벨 셀이 없어 삽입은 없음(정직
>   안내가 정답) — 라벨 매칭 삽입 자체는 Phase 3·스파이크에서 기검증. **트랙 소잔여 0**
>
> **후속 진행 (2026-07-08):**
