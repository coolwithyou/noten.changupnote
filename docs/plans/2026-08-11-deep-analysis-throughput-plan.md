...
2. **CP1**: ✅ T1 배선·재canary(손실 0건)·기본값 전환 완료(08-11 17:26, §2-3-4). 잔여: 다음 배치 1회에서 ① `primaryPasses`로 최빈 validator 실패 코드 표 수집(T4 2단계 입력) ② 2단 effort의 재판정 부하가 +15% 안쪽인지 배치 규모로 확인.
3. **CP2**: T4 2단계(최빈 코드 결정적 교정/프롬프트 승격) → repair 발생률 20% 미만 확인.
4. **CP3**: T2+T3+T5 구현(계약·엔진 버전 bump, §2.1 개정) → 신규 3건 + 기존 1건 재분석으로 품질 그래프 전 노드 passed 확인.
5. **CP4**: 20건 확대 배치로 종합 효과 실측 → 본 문서 §4 표를 실측으로 갱신.
