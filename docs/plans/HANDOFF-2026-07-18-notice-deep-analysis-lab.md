...
> ⚠️ **최신 스냅샷은 `docs/plans/HANDOFF-2026-07-21.md`** — 이 문서는 배경·이력 참조용.
...
**통과 기준(contract.ts의 ANALYSIS_LAB_GATES 상수로 확정)**: 엄격 정밀도(correct) ≥80% · 치명 오류율(wrong)
≤10% · 공고당 누락 ≤1건 · 커버리지(사람 확정 B ÷ 현행 A) ≥1.5x · 공고당 비용 ≤$1.
**보조 지표(e3123a1, 게이트 아님)**: 기계판정 가능 — 정확 확정 B 중 구조화(≠text_only) 비율 ·
공고당 기계판정 가능 criteria A→B 배수. 병목 진단(기계판정 가능률 ~40%)의 해소분을 직접 측정.
게이트 승격은 첫 집계 후 결정.
...
1. **[사용자만 가능] 검수 수행** — dev 서버는 사용자가 직접 기동(`pnpm dev:web`, 127.0.0.1:4010).
   ...
   **판정 실행 규칙은 `docs/research/2026-07-18-공모딥분석-검수판정-가이드.md`** — 리트머스
   질문·경계 규칙·공고별 최우선 확인 항목(★). 권장 순서: 원스톱 → 식품 → 통합공고.
