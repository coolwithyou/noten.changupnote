...
> 🟡 **진행 상황**: 실험실 구현·강화·검수 시트·집계 CLI까지 전부 커밋 완료(f4c542a).
...
1. **[사용자만 가능] 검수 수행** — dev 서버는 사용자가 직접 기동(`pnpm dev:web`, 127.0.0.1:4010).
   `/dev/analysis-lab` → 공고 카드의 **"런 선택" 드롭다운에서 런 선택**(이걸 해야 하단에 상세
   패널과 탭 4개가 나타남 — 직전 세션에서 사용자가 탭이 안 보인 원인이 "런 미선택") → "검수" 탭
   → 이메일(sw@ba-ton.kr) 입력 → 항목별 판정 → 맨 아래 **"검수 저장"** 클릭 → "마지막 저장 시각" 확인.
   런 선택 후에도 검수 탭이 없으면 브라우저 강력 새로고침(⌘⇧R — 구버전 번들).
