...
| 순서 | 작업 | 선행 의존 | 요점 |
|---|---|---|---|
| ① | ~~certification 룰 1차 구조화~~ **완료 (2026-07-06)** | — | 인증 사전 17종+canonical 매칭, 긍정→required/우대→preferred/애매→placeholder. required 채택분 전수 분류 오탐 0. 백필 2,901건 재발행(required 34·preferred 63, 실패 0). 기존 매처가 value.certs만 읽어 인증 조건이 평가되지 않던 잠복 버그 동시 수정. 잔존 placeholder 2,816건(가드 차단 1,103 포함)은 검수/LLM 2차 대상 |
| ② | ~~SMPP 자동 보강~~ **완료 (2026-07-06)** | — | 신청 당일 자동승인 확인. positive-only 병합(미보유 무변경, confidence.certification 미설정으로 부분 커버리지 오탈락 방지), 30일 캐시. dev 서버 실측: (주)바톤 여성기업확인서 자동 확정 — 충족도 6/10→**7/10** |
| ③ | **벤처·이노비즈·메인비즈 자동 보강** | 중소벤처24 API 승인 대기 중 (2026-07-06 신청 완료) | 가이드로 스펙·조회 허용 범위 확정 후 ②와 동일 패턴. 스펙 부적합·동의 제약 시 폴백: [벤처기업명단 파일데이터](https://www.data.go.kr/data/15084581/fileData.do) 월간 적재 |
| ④ | ~~엔드투엔드 검증~~ **완료 (①·② 범위)** | ③ 이후 재검증 | dev 티저 실측: summary에 SMPP 확인 문구, certification 필드 자동 확정 |
