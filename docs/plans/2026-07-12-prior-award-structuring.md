...
> 🟡 상태: 리뷰 반영 완료(재설계 지적 반영) (2026-07-12). 구현 미착수. codex(gpt-5.5 xhigh fast mode) 심층 리뷰 종합판정 "재설계" → 발견 7건 전건 반영(§리뷰 반영 기록) → 사용자 승인 후 별도 세션 구현.
...
evaluatePriorAward(criterion, company):
  value = adaptV1(criterion.value)   # v1 하위호환 어댑터(§4.4) — programs/awards/note/text_only 각 형식 흡수
...
§7.6 실측상 현행 37건은 `{programs}`뿐 아니라 `{awards}`·`labels`·`{note}`/`text_only`가 혼재한다(§4.1 원안 폴백은 `{programs}`만 처리 → 나머지 형식 오판정). v1 값을 신규 `PriorAwardCriterionValue`로 변환하는 **별도 함수**로 명시하고, 형식별 매핑을 강제한다. `match.ts:159-160`(호출부)·`match.ts:416-429`(기존 exact-match 경로) 대체 지점.

| v1 값 형식 | 실측 근거 | 신규 값 변환 규칙 |
|---|---|---|
| `{ programs: string[] }` | required/in·preferred/in(§7.6) | `{ scope:"program", programs: canonical(programs), labels: programs }`. canonical 실패 토큰은 labels로 보존 |
...
- P1 완료 기준에 형식별(programs/awards/note) golden 회귀를 추가한다.
...
- `evaluatePriorAward` 신설(§4). `evaluateCriterion` switch의 `prior_award` 케이스(match.ts:159-160)를 교체. **`adaptV1` v1 하위호환 어댑터(§4.4) 포함** — `{programs}`·`{awards}`·`labels`·`{note}`/`text_only` 형식 전수 흡수.
- `labelFor`·`nextQuestion` prompts(match.ts:1143 기존 "동일하거나 유사한 정부지원사업 선정 이력이 있나요?")를 self_kind·채널 구분 문항으로 개정(§6).
- `RULESET_VERSION` 범프(`ruleset-kstartup-spine-v4`) — 매칭 스냅샷 재계산 트리거(§P7).

**완료 기준**: evaluator 테스트 매트릭스(§4.2 전 행) + **37건 형식별(programs/awards/note/text_only) golden 회귀**(B3) + 기존 golden 회귀 100%. **방어층 미해제** — 이 시점 DB엔 아직 prior_award 구조화 criterion이 없으므로 evaluator는 v1 어댑터만 실행(무해).
...
- **v1 잔존 청산**(§7.1 실측): `bizinfo-llm-criteria-v1` 잔존 prior_award exclusion 3건(활성 open 공고, 그중 1건 `not_in`=현행 false-pass 위험) 재발행 대상 확인.
- match_state 재계산(`ruleset-kstartup-spine-v4`).
...
`grant_criteria` 87,770행 / `grants` 31,388행. prior_award dimension 37행(전량 kind/operator 분포 §7.6).

### 7.1 prior_award + exclusion 잔존 3건 (방어층 우회 진단)

전량 `bizinfo-llm-criteria-v1`(구 파서, 방어층 도입 전). **전부 활성(`open`)** 공고 — 매칭 우주 안. 그중 1건이 `not_in`(=구조화 위반, **현행 false-pass 위험**):
...
### 7.6 현행 prior_award 37건 구성 (하위호환 대상)

`required/text_only` 15, `preferred/text_only` 8, `preferred/exists` 3, `preferred/in` 3, `required/in` 2, `required/exists` 2, `exclusion/text_only` 2, `exclusion/not_in` 1, `preferred/not_in` 1. required/exists 사례("전북도 통상닥터 참여기업이어야 함") = 전제 이력 필수(§4.2 required 행). v1 `{programs}`·`{awards}`·`{note}` 값 형식 혼재 → evaluator 폴백이 흡수.
...
| **v1 잔존 exclusion 미청산**(§7.1, not_in 1건 활성) | P6 백필이 v1 잔존을 재발행 대상에 포함. 완료 기준에 "v1 prior_award exclusion 0" 명시 |
...
| 저장된 매칭 스냅샷 혼재 | P6 재계산 + `ruleset-kstartup-spine-v4` 범프 |
...
9. 기존 golden 회귀 100%: **37건 형식별(programs/awards/note/text_only) `adaptV1` 어댑터 무회귀**(B3, §4.4).
...
| B3 하위호환 폴백이 실측 형식(`{programs}`·`{awards}`·`{note}`/`text_only`) 미포괄 | Major | 신설 §4.4 `adaptV1` 별도 어댑터 함수 + 형식별 변환 규칙 표. §4.1은 `adaptV1(value)` 호출로 변경. P1 완료 기준에 "37건 형식별(programs/awards/note/text_only) golden 회귀" 추가. §9-9 개정 |
