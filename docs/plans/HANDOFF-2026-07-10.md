...
1. ~~**env 키 세팅**~~ — **완료 (2026-07-10 후속 세션)**:
   ...
   - env는 **다음 배포부터** 주입됨 (2026-07-10 현재 프로덕션은 크레딧 코드 자체가 미배포 — /pricing·/credits·크레딧 cron 전부 404 확인)
...
5. **배포 시**: `vercel.json`의 크레딧 cron 5종(실측 **총 9종** — 본 문서의 "10종"은 오기) 플랜 제한 확인 → **noten 팀 pro 플랜 확인 완료(한도 40, 분 단위 스케줄 허용 — 문제 없음)**. 신규 env는 Vercel에 등록 완료라 배포만 하면 반영. 배포 규칙은 프로젝트 CLAUDE.md(Vercel CLI 토큰) 준수. admin(changupnote-ops)도 신규 env 주입 위해 재배포 필요
