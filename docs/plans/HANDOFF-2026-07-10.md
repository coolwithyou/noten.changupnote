...
1. **env 키 세팅** — `.env.local`(로컬)과 Vercel 프로젝트(changupnote·changupnote-ops) 양쪽:
   - `INTERNAL_API_SECRET` (웹·admin **동일 값** — admin→웹 내부 호출 인증), `WEB_INTERNAL_BASE_URL` (admin에 — 로컬 `http://localhost:3000`, 운영은 웹 도메인)
   - `CRON_SECRET`, `CREDIT_BIZNO_HMAC_PEPPER` (팝빌 사업자번호 가명 키)
   - 키 목록·설명은 `.env.example` 크레딧 섹션 참조
...
5. **배포 시**: `vercel.json`의 크레딧 cron 5종(총 10종) 플랜 제한 확인 + 신규 env 반영. 배포 규칙은 프로젝트 CLAUDE.md(Vercel CLI 토큰) 준수
6. **로컬 정리(rm은 사용자)**: `.tmp-*`·`.done-*` 마커 3개(`packages/core/scripts/`, `apps/web/src/lib/server/{db,credits}/`), `.git/stale-locks/`
...
- [ ] env 4종 로컬·Vercel 세팅 (INTERNAL_API_SECRET / WEB_INTERNAL_BASE_URL / CRON_SECRET / CREDIT_BIZNO_HMAC_PEPPER)
- [ ] 포트원 단건 E2E 1회 완주 (C4: 결제→지급→웹훅 멱등→콘솔 취소 회수)
- [ ] 실채널 첫 웹훅에서 서명 헤더 형식 확인 (표준 Webhooks 스펙 가정 구현 — 다르면 `portoneWebhook.ts` 조정)
- [ ] 토스 빌링 E2E (구독→갱신→해지)
- [ ] admin F1~F3 런타임 확인
- [ ] 팝빌 실호출 미터링 1건 확인
- [ ] 배포 후 cron 10종 동작 + 대사 5 scope ok (portone scope는 키 세팅 후 ok로 바뀌어야 정상)
