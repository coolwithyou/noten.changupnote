...
> - 구현: **다음 작업 = P3 (충전 결제)**. 코드는 키 없이 구현 가능 — E2E(C4)만 포트원 키 필요
> - 미확인: ① 팝빌 실호출 미터링(B2)은 코드경로만 검증 — 실호출 1건은 사용자 환경에서 ② CRON_SECRET·CREDIT_BIZNO_HMAC_PEPPER 실환경 값 설정 필요 ③ 이메일 인증 플로우 부재로 로그인 성공=인증 완료 해석(6.6)
