...
> - **P1 완료** (커밋 a6d4f62·b7f6fd6·33e0a4a): 스키마 14테이블+0038 마이그레이션 적용, core credits 도메인+리포지토리, 가입 보너스 훅, 시드 실행, verify I1~I10 OK. A1~A6 검수 통과. RLS 실측: postgres 역할 rolbypassrls=true → 전 테이블 ENABLE+FORCE 분기
> - 구현: **다음 작업 = P2 (과금 파이프라인). P6은 병행 가능**
> - 미해결: ① backfill-signup-bonus.ts 작성만 됨 — 실 DB 지급 시점은 사용자 결정 ② 6.6의 "잔액 첫 조회 안전망" 배선은 잔액 API 생기는 P2에서 ③ 이 코드베이스에 이메일 인증 플로우가 없어 로그인 성공을 인증 완료 지점으로 삼음(6.6 해석)
> - 사용자 준비물 상태: 포트원 계정·채널 키 미확보 (P3 착수 전 필요 — §5)
> - 마지막 갱신: 2026-07-10 (P1 완료 세션)
