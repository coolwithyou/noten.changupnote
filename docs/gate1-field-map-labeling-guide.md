...
배치 3(2026-07-02, 16문서 530필드, `spike-labels/batch3-report.md`)에서 확정한 규칙:

7. **체크박스 없는 동의문·서약문은 signature 필드 1개** — "~에 동의합니다" 서술 후 서명만 요구하는 형태는 동의 행위가 서명으로 귀결되므로 `type: signature, manual: true`로 라벨하고 notes에 동의문임을 기록한다. 별도 checkbox 필드를 만들지 않는다.
8. **자가진단 체크리스트의 manual 판정 기준 = 법적 책임 문구** — "허위 기재 시 불이익/취소" 등 책임 문구가 동반되거나 서명으로 확인하는 자가 체크표는 서약성이므로 `manual: true`. 단순 준비물 확인용 정보성 체크는 `manual: false`.
9. **겸용 셀(두 의미가 한 칸)은 주 용도 key 하나** — 예: 생년월일/사업자번호 겸용 칸은 신청 주체 기준의 주 용도로 key를 정하고 notes에 겸용임을 기록. 검수 시 문서 계열별 canonical key로 통일한다.

배치 4(2026-07-02, PDF/DOCX 14문서 398필드, `spike-labels/batch4-report.md`)에서 확정한 규칙:

10. **언어 병기 택1 서식은 한국어판만 인스턴스로 라벨** — 같은 서식의 한/영 병렬 제공 시 한국어판 필드만 세고, 영문판은 notes에 병렬 존재를 기록한다 (규칙 2의 확장 — 배타 서식과 동일하게 매칭 단위는 인스턴스).
